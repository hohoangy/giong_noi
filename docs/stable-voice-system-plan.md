...
3. Chạy phrase/corpus matcher để lấy top candidates.
4. Chạy AI arbiter nếu có `OPENAI_API_KEY`.
5. Decision engine chọn một trong bốn trạng thái:
   - `use_raw`: dùng raw STT.
   - `use_guess`: dùng câu đoán.
   - `ask_user`: hỏi lại trước khi học/phát.
   - `learn_new_phrase`: lưu phrase cá nhân mới.
6. Review đúng/sai cập nhật phrasebook cá nhân và trust, không cập nhật bừa corpus matcher.
